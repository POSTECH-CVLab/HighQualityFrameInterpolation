...
2. Put your video (.mp4 file) in ```data``` folder
3. Make a image sequence from a video using ```python vid2frames.py```
4. Run ```CUDA_VISIBLE_DEVICES=0 python run_seq.py --cuda --n_iter 1 --in_file data/[your_video_name] --out_file data/[output_folder_name]```
5. Check the video folder (```custom/your_video_name/```) and generated images. ```xx_1.png``` indicates interpolated frames, and ```xx_0.png``` indicates original video frames.


## Training the Network
1. Compile PWCNet in correlation package. ```mv models/PWCNet/correlation_package_pytorch1_0/ && build.sh```. If compilation is not working consider modifying ```setup.py``` and change the line ```'-gencode', 'arch=compute_75,code=sm_75'``` with the proper GPU version by referencing [this](https://arnon.dk/matching-sm-architectures-arch-and-gencode-for-various-nvidia-cards/).
